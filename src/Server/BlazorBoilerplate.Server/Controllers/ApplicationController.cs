...
namespace BlazorBoilerplate.Server.Controllers
{
    ...
    {
        private const string AuthSchemes = "Identity.Application" + "," + JwtBearerDefaults.AuthenticationScheme; //Cookie + Token authentication

        private readonly ApplicationPersistenceManager persistenceManager;
        public ApplicationController(ApplicationPersistenceManager persistenceManager) => this.persistenceManager = persistenceManager;

        [AllowAnonymous]
        [HttpGet]
        public string Metadata() => persistenceManager.Metadata();
        
        [HttpGet]
        public Task<UserProfile> UserProfile() => persistenceManager.GetUserProfile();
        
        [AllowAnonymous]
        [HttpGet]
        public IQueryable<TenantSetting> TenantSettings() => persistenceManager.GetEntities<TenantSetting>().AsNoTracking();
        
        [HttpGet]
        [Authorize(Policies.IsAdmin)]
        public IQueryable<ApplicationUser> Users() => persistenceManager.GetEntities<ApplicationUser>().AsNoTracking().Include(i => i.UserRoles).ThenInclude(i => i.Role).OrderBy(i => i.UserName);

        [HttpGet]
        [Authorize(Policies.IsAdmin)]
        public IQueryable<ApplicationRole> Roles() => persistenceManager.GetEntities<ApplicationRole>().AsNoTracking().OrderBy(i => i.Name);
        ...
        [HttpGet]
        [Authorize(Policies.IsAdmin)]
        public IQueryable<DbLog> Logs() => persistenceManager.GetEntities<DbLog>().AsNoTracking().OrderByDescending(i => i.TimeStamp);
        
        [HttpGet]
        [Authorize(Policies.IsAdmin)]
        public IQueryable<ApiLogItem> ApiLogs() => persistenceManager.GetEntities<ApiLogItem>().AsNoTracking().OrderByDescending(i => i.RequestTime);
